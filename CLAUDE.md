...
- 14차: **디자인 리파인**(design-taste-frontend 스킬, Redesign-Preserve) — Phase0 감사(`docs/design-audit.md`), Phase1 폰트 self-host(Pretendard 가변 한글전체음절 서브셋·Caveat, `@font-face`+swap, CDN·CSP에서 jsdelivr/구글 제거)+디자인 토큰(radius 3+pill·shadow 3·spacing·타입스케일), Phase2 이모지 chrome→**Phosphor `<symbol>` 스프라이트**+`ic()` 헬퍼(카테고리·배지·크레용·토스트 이모지·필기체는 유지, 잘림방지 `.ic`), Phase3 대비 AA(`--muted`#736858·`--carrot-deep`#B5460D)·포커스 링 통일·`prefers-reduced-motion`·hover lift. (이메일 리디자인 Phase4는 보류)
